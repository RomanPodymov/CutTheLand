...
function Enemy()
	...
	function self.createEnemy (initial_position_x, intial_position_y, initialIndexI, initialIndexJ, size, background, eventsTimeInterval)
	...
	function self.onMove ( )
        ...
        if not (self.locked) then
            ...
            if self.moveDirection == MOVE_DIRECTION_LEFT then
                ...
                if self.locked then
                    self.moveDirection = MOVE_DIRECTION_RIGHT
                    self.background.tryToUnlockEnemy(self.indexI, self.indexJ, self.moveDirection, self)
                    if self.locked then
                        self.moveDirection = MOVE_DIRECTION_DOWN
                        self.background.tryToUnlockEnemy(self.indexI, self.indexJ, self.moveDirection, self)
            ...
            elseif self.moveDirection == MOVE_DIRECTION_RIGHT then
                ...
                self.background.tryToUnlockEnemy(self.indexI, self.indexJ, self.moveDirection, self)
                if self.locked then
                    self.moveDirection = MOVE_DIRECTION_LEFT
                    self.background.tryToUnlockEnemy(self.indexI, self.indexJ, self.moveDirection, self)
                    if self.locked then
                        self.moveDirection = MOVE_DIRECTION_UP
                        self.background.tryToUnlockEnemy(self.indexI, self.indexJ, self.moveDirection, self)
